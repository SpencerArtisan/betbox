...
class Market
  ...
  def odds
    results = prices
    puts "#{results[0][:b3]}|#{results[0][:b2]}|#{results[0][:b1]}"
    puts "#{results[1][:b3]}|#{results[1][:b2]}|#{results[1][:b1]}"
    OpenStruct.new player_a_win: results[0][:b1], player_b_win: results[1][:b1]
  end
